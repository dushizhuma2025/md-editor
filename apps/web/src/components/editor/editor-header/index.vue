...
import { Bot, Copy, Image as ImageIcon, Menu, Monitor, Palette, Smartphone } from 'lucide-vue-next'
...
function togglePreviewMode() {
  const isMobileMode = previewWidth.value === 'w-[375px]'
  previewWidth.value = isMobileMode ? 'w-full' : 'w-[375px]'
}
</script>
...
<template>
  ...
  >
    ...
    <div class="flex items-center">
      ...
      <div class="flex items-center ml-8 hidden md:flex">
        <Menubar class="menubar border-0">
          ...
          <InsertDropdown />
          <ViewDropdown />
          <AdvancedDropdown @open-about="handleOpenAbout" />
          <MenubarSeparator class="mr-4" />
          <Button
            variant="ghost"
            size="sm"
            @click="openAIChat"
    ...
    <div class="flex flex-wrap items-center gap-2">
        ...
        <Menubar class="menubar border-0 p-0">
          <MenubarMenu>
            ...
            <MenubarContent align="start">
              ...
              <AdvancedDropdown :as-sub="true" @open-about="handleOpenAbout" />
              <MenubarSeparator />
              <MenubarItem @click="openAIChat">
                <Bot class="mr-2 h-4 w-4" />
                AI写作助手
              </MenubarItem>
              <MenubarItem @click="openAIImageGenerator">
                <ImageIcon class="mr-2 h-4 w-4" />
                AI绘图助手
              </MenubarItem>
            </MenubarContent>
      ...
      >
        <Monitor v-if="previewWidth === 'w-[375px]'" class="mr-2 h-4 w-4" />
        <Smartphone v-else class="mr-2 h-4 w-4" />
        <span>{{ previewWidth === 'w-[375px]' ? '电脑预览' : '手机预览' }}</span>
      </Button>
    </div>
  </header>
